...
export default function SearchBar({ value, onChange, large }: Props) {
	return (
		<div className="relative">
			...
			<input type="text" value={value} onChange={(e) => onChange(e.target.value)} placeholder="Search clipboard history" className={`selectable bg-bg-secondary text-text-primary border-border focus:border-accent placeholder:text-text-muted w-full rounded-[var(--radius-card)] border transition-colors focus:outline-none ${large ? "py-2.5 pr-10 pl-10 text-[14px]" : "py-1.5 pr-8 pl-8 text-[12px]"}`} />
			{value && (
